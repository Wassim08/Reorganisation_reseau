...
Noeud *rechercheNoeudH(TableHachage *H, double x, double y, int *pos)
{
    ...
    if (!H)
    { // On verifie si la Table est bien definie
        fprintf(stderr, "Erreur : La Table de hachage n'est pas defini\n");
        *pos = -1;
        return NULL;
    }
    ...
    *pos = fonction_hachage(H, clef);
    ...
    if (*pos >= H->taille)
    { // on verifie que la position existe dans la table
        fprintf(stderr, "Erreur : La table de hachage n'est pas assez grande\n");
        *pos = -2;
        return 0;
    }
    NoeudH **H_Lnoeud = &(H->tab[*pos]);
...
Noeud *rechercheCreeNoeudHachage(Reseau *R, TableHachage *H, double x, double y)
{
    ...
    if (!R)
    {   // On verifie si le Reseau est bien definie
...
TableHachage *creerTableHachage(int taille)
{
    ...
    if (!H)
    {
        fprintf(stderr, "Erreur lors de l'allocation de l'espace pour la table de hachage 1\n");
        return NULL;
    }
    H->taille = taille;
